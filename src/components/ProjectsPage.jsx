...
const ProjectsPage = React.memo(
  ({ t, lang, projects, isAdmin, handleEditProject, handleDeleteProject }) => {
    ...
    const finalProjects =
      projects.length > 0
        ? projects
        : t.projects.list.map((dt) => ({ ...dt, ...projectDefaults[dt.id] }));

    const displayedProjects = showAll
      ? finalProjects
      : finalProjects.slice(0, 4);

    const getBentoClass = (i) => {
      if (i === 4) return "md:col-span-2 md:row-span-2";
      if (i === 5) return "md:col-span-1 md:row-span-2";
      if (i === 6) return "md:col-span-1 md:row-span-2";
      ...
      >
        ...
        <div className="text-center mb-16 relative z-20 px-4">
          <h3 className="text-4xl font-bold mb-4 text-white animate-fade-up">
            <span key={`${lang}-proj-title`}>{t.projects.title}</span>{" "}
            <span className="text-teal-400"> {t.projects.subtitle}</span>
          </h3>
          <p
            key={`${lang}-proj-desc`}
        ...
        <div
          ...
          ref={sectionRef}
        >
          <div className="grid grid-cols-1 md:grid-cols-4 auto-rows-[200px] gap-4 transition-all duration-500">
            {displayedProjects.map((project, index) => {
              ...
              const isSmallGrid =
                bentoClass.includes("col-span-1") &&
                bentoClass.includes("row-span-1");

              return (
                <div
                  key={project.id}
                  className={`relative group rounded-3xl overflow-hidden border border-white/10 shadow-lg transition-all duration-700 hover:border-teal-500/50 ${bentoClass} ${
                    displayedProjects.includes(project)
                      ? "opacity-100 translate-y-0"
                      : "opacity-0 translate-y-8"
                  }`}
                  style={{
                    transitionDelay: displayedProjects.includes(project)
                      ? `${displayedProjects.indexOf(project) * 100}ms`
                      : "0s",
                  }}
                >
                  <Wrapper
                  ...
                  >
                    ...
                    <div className="absolute inset-0 p-6 flex flex-col justify-end translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                      ...
                      <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-75">
                        ...
                        <h4
                          className={`font-bold text-white mb-2 leading-tight ${isSmallGrid ? "text-lg" : "text-2xl"}`}
                        >
                          {project.title}
                        </h4>
                        ...
                        <p
                          className={`text-gray-300 mb-4 ${isSmallGrid ? "text-xs line-clamp-2" : "text-sm line-clamp-3"}`}
                        >
                          {project.desc}
                        </p>
                        ...
                        <div className="flex items-center text-sm font-medium text-teal-400">
                          <span>
                            {t.projects.btnAll.includes("All")
                              ? "View"
                              : "Lihat"}
                          </span>
          ...
          <div className="mt-16 text-center transition-all duration-500">
            {finalProjects.length > 4 && (
              <button
                onClick={() => setShowAll(!showAll)}
                className="group px-8 py-3 rounded-full border border-gray-700 text-gray-300 hover:bg-teal-500/10 hover:text-teal-400 hover:border-teal-500/50 transition-all duration-300"
              >
                <span
                  key={`${lang}-view-all`}
